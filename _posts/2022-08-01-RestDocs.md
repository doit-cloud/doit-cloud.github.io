...
파일의 이름은 원하는대로 설정 가능하며, <ww>build.gradle</ww>에 설정한 경로인 <ww>src/resources/static/docs</ww> 에 같은 이름의 html파일로 복사됩니다.
...
빌드 실패 시 <ww>build/reports/tests/test/index.html</ww>에서 테스트 결과와 실패 원인을 확인해 볼 수 있습니다.
...
(o)<ww>operation::user[snippets='http-request,http-response,response-fields']</ww>
(x)<ww>operation::user[snippets='http-request, http-response, response-fields']</ww>
...
기본 템플릿은 <ww>spring-restdocs-core</ww>의  <ww>org.springframework.restdocs.templates.asciidoctor</ww>에 존재합니다.
