...
Webpack, Babel 같은거 없이도 지금까지 잘쓰고 있지 않던가 싶을 수도 있죠.

하지만 대부분의 경우 여러분은 이미 Webpack, Babel을 사용하고 있어요.

CRA(Create-React-App)나 다른 보일러 템플릿이 자동으로 구성해줬기에 모르고 있었을 뿐이죠.

내가 숨쉬듯 사용하던 세팅에 뭔지 모를게 섞여있다면 한번쯤은 알아봐야 하지 않을까요?
...
2차 브라우저 전쟁 시기, **IE**는 **ES6**가 제정되었지만 `JScript`만의 문법을 사용하는 등 여전히 독자적인 노선을 걸었습니다.

하지만 **ES6**에는 새롭게 등장한 혁신적인 기능이 많았기 때문에, 당시의 개발자들은 방법을 강구해야 했죠.

**Babel**의 원래 이름은  **6to5** 입니다.

그 이름에서 짐작 가능하시겠지만, Babel은 **ES6(ECMAScript 6)** 를 **ES5(ECMAScript 5)** 로 변환하는 컴파일러로서 등장했습니다.
...
Web의 규모가 커져나가고 대규모 프로젝트가 하나 둘 만들어지던 시절,

데이터의 양, 사용자 수, 웹 복잡성들이 모두 증가했기에 자원을 관리해야할 필요성에 의해 등장하게 되었습니다.

이런 흐름에 따라 Web 개발 시장에서 모듈 번들러에게 요구된 것은, 크게 3가지 이슈입니다.
...
따라서 빌드시 중복선언으로 인한 문제가 무척 다분했고, js 파일을 모듈로서 관리해야 할 필요성이 있었습니다.
